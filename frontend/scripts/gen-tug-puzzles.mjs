...
// --- Tactical themes (fork / pin / skewer) ---------------------------------
// The mate set above collapses to one puzzle shape inside a rating band, so the
// offline fallback could never honor a "fork"/"pin"/"skewer" theme filter. Each
// base below is a pawnless K+piece position whose short forced line wins
// material via the named motif; symmetry expansion (safe for pawnless boards)
// multiplies it, and the variants are spread across the Pawn/Knight/Rook rating
// bands so every band has a few puzzles per theme. Verified move-by-move with
// chess.js, then re-checked by __tests__/puzzles.test.ts.

// Verify a tactical (non-mate) line: legal white-to-move FEN, black not already
// in check, an odd-length line (solver plays first and last) with every move
// legal. Returns { fen, moves } or null.
function verifyLine(pieces, moves) {
  const fen = makeFen(pieces);
  let c;
  try { c = new Chess(fen); } catch { return null; }
  if (c.turn() !== 'w' || c.isCheck()) return null;
  if (moves.length % 2 !== 1) return null;
...
// Expand a tactical base into its verifying symmetric variants, dedup by FEN.
function expandLine(pieces, moves) {
  const seen = new Set();
  const out = [];
  for (const sym of SYMMETRIES) {
    const p = {};
...
const TACTIC_BASES = [
  // Knight fork: Ne7+ forks the king (g8) and rook (c8); after the king steps
  // away the knight takes the rook.
  { theme: 'fork', pieces: { g8: 'k', c8: 'r', d5: 'N', g1: 'K' }, moves: ['d5e7', 'g8h8', 'e7c8'] },
  // Pin: the black queen is pinned to its king down the a-file by Ra1; the
  // bishop wins the pinned queen outright (Ra1 discovers check as a bonus).
  { theme: 'pin', pieces: { a8: 'k', a5: 'q', a1: 'R', e1: 'B', g1: 'K' }, moves: ['e1a5'] },
  // Skewer: Re8+ skewers the king (b8) to the queen behind it (a8); the king
  // steps off the back rank and the rook collects the queen.
  { theme: 'skewer', pieces: { b8: 'k', a8: 'q', e1: 'R', g1: 'K' }, moves: ['e1e8', 'b8b7', 'e8a8'] },
];

// The rating bands the tactical variants are spread across (kept ≤1400 so the
// classroom set stays beginner-friendly, matching puzzles.test.ts).
const TACTIC_BANDS = [
  { rating: 700, prefix: 'p' },   // Pawn band (400–800)
  { rating: 1000, prefix: 'k' },  // Knight band (800–1200)
  { rating: 1300, prefix: 'r' },  // Rook band (1200–1600)
];
...
// Fan the tactical variants round-robin across the bands so each band gets a
// few of every theme.
for (const base of TACTIC_BASES) {
  const variants = expandLine(base.pieces, base.moves);
  variants.forEach((v, i) => {
    const band = TACTIC_BANDS[i % TACTIC_BANDS.length];
    puzzles.push({
      id: `tug-${base.theme}-${band.prefix}-${pad(++n)}`,
      fen: v.fen,
      moves: v.moves,
      rating: band.rating,
      themes: [base.theme],
  ...
  console.error(`tactic ${base.theme}: ${variants.length} variants`);
}
