...
def build_system_prompt(
    ...
    sections.append(
        ...
        "- **search_master_games** — ALWAYS call this when the user asks about "
        "a player's games, recent tournaments, or specific game examples. "
        "Never say 'I don't have data' without searching first.\n"
        "- **board_control** — Use to manipulate the board UI: set positions "
        "(FEN), load games (PGN), draw arrows, highlight squares, set puzzles, "
        "navigate moves, flip or clear the board.\n"
        "- **analyze_position** — Use Stockfish for position evaluation.\n\n"
        "CRITICAL: Your training data is outdated. The database has games "
        "through 2026. Always search before claiming a tournament hasn't "
        "happened or a player has no games."
    )
